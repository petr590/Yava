...
public final class StackChanging {

	private StackChanging() {}
	...
	public static void dup2(DecompilationContext context) {
		...
		switch (value.getReturnType().getSize()) {
			case WORD -> context.push(context.peekWithSize(1, TypeSize.WORD));
			case LONG -> {}
			default   ->
					throw new DecompilationException("Illegal operation type size " + value.getReturnType().getSize());
		}
	...
	private static void dup2X(DecompilationContext context, int words, String instructionName) {
		...
		switch (value1.getReturnType().getSize()) {
			case WORD -> context.push(values.get(1));
			case LONG -> {}
			default   ->
					throw new DecompilationException("Illegal operation type size " + value1.getReturnType().getSize());
		}
	...
	public static void swap(DecompilationContext context) {
		Operation value1 = context.popWithSize(TypeSize.WORD);
		Operation value2 = context.popWithSize(TypeSize.WORD);
