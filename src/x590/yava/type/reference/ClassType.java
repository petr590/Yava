...
import static x590.util.io.UncheckedInputStream.EOF_CHAR;
...
public class ClassType extends RealReferenceType {
	...
	private enum ClassKind {
		...
		ClassKind(String name, boolean isSpecial) {
			this.name = name;
	...
	private ClassType(ExtendedStringInputStream in) {
		...
		Loop:
		for (int i = 0, ch, prevCh = 0; ; i++, prevCh = ch) {
			ch = in.read();
	...
	@Override
	public String toString() {
		return "class " + (signature.isEmpty() ? name : name + signature);
	}
	 ...
	 * для всех остальных классов - {@code this}
	 */
