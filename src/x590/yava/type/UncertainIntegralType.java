 ...
 * размером не больше 32 бит ({@code byte}, {@code short}, {@code int}, {@code char}, {@code boolean}).
 * Хранит минимальный и максимальный размер типа, а также два флага,
 * которые отвечают за типы {@code boolean} и {@code char}.
 */
public final class UncertainIntegralType extends Type {

	private static final UncertainIntegralType[] INSTANCES =
			new UncertainIntegralType[1 << 6 /* minCapacity - 2 бита, maxCapacity - 2 бита, флаги - 2 бита */];
	...
	private static PrimitiveType primitiveTypeByCapacity(int capacity, boolean includeChar) {
		...
		return switch (capacity) {
			...
			case 2 -> SHORT;
			case 4 -> INT;
			default -> throw new IllegalArgumentException("Cannot find " + (includeChar ? "unsigned" : "signed") +
	...
	private static Type getInstanceNoexcept(int minCapacity, int maxCapacity, int flags) {
		...
		if (minCapacity == maxCapacity && flags == NONE_FLAGS)
	...
	public static Type getInstance(int minCapacity, int maxCapacity) {
		return getInstance(minCapacity, maxCapacity, NONE_FLAGS);
	}
	...
	public static Type getInstance(PrimitiveType widestType, PrimitiveType narrowestType) {

		int capacity1 = 0, capacity2 = 0;
		boolean includeBoolean = false, includeChar = false;

		if (widestType == BOOLEAN) {
			includeBoolean = true;
		} else if (widestType == CHAR) {
			includeChar = true;
		} else if (widestType instanceof IntegralType widestIntegralType) {
			capacity1 = widestIntegralType.getCapacity();
		} else {
			throw new IllegalTypeException("widestType = " + widestType);
		}

		if (narrowestType == BOOLEAN) {
			includeBoolean = true;
		} else if (narrowestType == CHAR) {
			includeChar = true;
		} else if (narrowestType instanceof IntegralType integralType) {
			capacity2 = integralType.getCapacity();
		} else {
			throw new IllegalTypeException("widestType = " + widestType);
		}

		if (widestType == narrowestType) {
			return widestType;
		}
		...
		if (capacity1 == 0) { // capacity2 == 0
			throw new IllegalTypeException("widestType = " + widestType + ", narrowestType = " + narrowestType);
		}

		return getInstance(Math.min(capacity1, capacity2), Math.max(capacity1, capacity2),
				includeBoolean, includeChar);
	}
