	...
	public static class TypeAnnotation implements StringifyWritable<ClassInfo>, Importable {
		...
		public TypeAnnotation(ExtendedDataInputStream in, ConstantPool pool, Location location) {
			...
			this.target = switch (location) {
				case CLASS -> switch (tag) {
					case 0x00 -> new TypeParameterTarget(in, pool);      // type parameter declaration of generic class
					case 0x10 ->
							new SupertypeTarget(in, pool);          // type in extends or implements clause of class declaration
					case 0x11 ->
							new TypeParameterBoundTarget(in, pool); // type in bound of type parameter declaration of generic class

					default -> throw newInvalidTargetTypeException(tag);
				};

				case FIELD -> switch (tag) {
					case 0x13 -> EmptyTarget.INSTANCE;                   // type in field declaration

					default -> throw newInvalidTargetTypeException(tag);
				};

				case METHOD -> switch (tag) {
					case 0x01 -> new TypeParameterTarget(in, pool);      // type parameter declaration of generic method
					case 0x12 ->
							new TypeParameterBoundTarget(in, pool); // type in bound of type parameter declaration of generic method
					case 0x14 ->
							EmptyTarget.INSTANCE;                   // return type of method, or type of newly constructed object
					case 0x15 -> EmptyTarget.INSTANCE;                   // receiver type of method or constructor
					case 0x16 ->
							new FormalParameterTarget(in, pool);    // type in formal parameter declaration of method, constructor, or lambda
					case 0x17 ->
							new ThrowsTarget(in, pool);             // type in throws clause of method or constructor

					default -> throw newInvalidTargetTypeException(tag);
				};

				case CODE_ATTRIBUTE -> switch (tag) {
					...
					case 0x45 ->
							new OffsetTarget(in, pool);             // type in method reference expression using ::new
					case 0x46 ->
							new OffsetTarget(in, pool);             // type in method reference expression using ::Identifier
					case 0x47 -> new TypeArgumentTarget(in, pool);       // type in cast expression
					case 0x48 ->
							new TypeArgumentTarget(in, pool);       // type argument for generic constructor in new expression or explicit constructor invocation statement
					case 0x49 ->
							new TypeArgumentTarget(in, pool);       // type argument for generic method in method invocation expression
					case 0x4A ->
							new TypeArgumentTarget(in, pool);       // type argument for generic constructor in method reference expression using ::new
					case 0x4B ->
							new TypeArgumentTarget(in, pool);       // type argument for generic method in method reference expression using ::Identifier

					default -> throw newInvalidTargetTypeException(tag);
				};

				default -> throw new DisassemblingException("Invalid location" +
