...
import x590.yava.type.CastStatus;
 ...
 */
public abstract sealed class ConstableValueConstant<T extends Constable>
		extends ConstValueConstant
		permits IntegerConstant, SingleConstableValueConstant {
	...
	public final void writeTo(StringifyOutputStream out, ClassInfo classinfo, Type type) {
		writeTo(out, classinfo, type, StringUtil.NONE);
	}

	@Override
	public final void writeTo(StringifyOutputStream out, ClassInfo classinfo, Type type, int flags) {
		...
		if (constantField != null) {
			...
			this.writeValue(out, classinfo, type, flags, null);
		}
	}


	protected static boolean canUseConstants() {
		return Yava.getConfig().constantsUsagePolicy() != Config.UsagePolicy.NEVER;
	}
	...
	public abstract void writeValue(StringifyOutputStream out, ClassInfo classinfo, Type type, int flags, @Nullable FieldDescriptor ownerConstant);
