...
import x590.yava.attribute.code.ExceptionTable;
import x590.yava.attribute.code.ExceptionTable.CatchEntry;
...
public final class DecompilationContext extends DecompilationAndStringifyContext implements Importable {
	...
	private final DecompilationStats stats;


	public static DecompilationContext decompile(Context otherContext, ClassInfo classinfo, JavaMethod method, DecompilationStats statistics, List<Instruction> instructions) {
		return new DecompilationContext(otherContext, classinfo, method, statistics, instructions);
	}

	private DecompilationContext(Context otherContext, ClassInfo classinfo, JavaMethod method, DecompilationStats stats, List<Instruction> instructions) {
		...
		final List<TryEntry> tryEntries = new ArrayList<>(exceptionTable.getEntries());
		...
		this.expressionIndexTable = new Int2IntArrayMap(instructions.size());
...
//			);
	...
	private void startScopes() {
		scopesQueue.removeIf(this::startScope);
	}
...
//				"At index " + index + ": " + scope + " must have parent scope " + currentScope + ", not " + scope.superScope();
	...
	/**
	 * @return Первое состояние стека по индексу или {@code null}, если его нет
	 */
	 ...
	 *
	 * @return Удалённое состояние стека или {@code null}, если удалять нечего.
	 */
	...
	public DecompilationStats getStats() {
		return stats;
	}
