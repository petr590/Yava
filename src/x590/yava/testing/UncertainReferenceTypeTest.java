...
import x590.yava.type.reference.generic.DefiniteGenericType;
...
//		assertNull(objectNumber.castToNarrowestNoexcept(UncertainReferenceType.getInstance(string)));
	...
	public void testWithArrayTypes() {
		...
		assertEquals(PrimitiveType.BYTE_BOOLEAN,
				((IArrayType)UncertainReferenceType.getInstance(ArrayType.BYTE_OR_BOOLEAN_ARRAY, ArrayType.BYTE_ARRAY))
						.getElementType());

		assertEquals(
				UncertainReferenceType.getInstance(ClassType.OBJECT, ArrayType.STRING_ARRAY),
				((IArrayType)UncertainReferenceType.getInstance(ArrayType.OBJECT_ARRAY, ArrayType.STRING_ARRAY.arrayType()))
						.getMemberType()
		);
	}

	@Test
	public void testArrayForType() {
		Type type1 = UncertainReferenceType.getInstance(ClassType.OBJECT, ArrayType.STRING_ARRAY);
		Type type2 = UncertainReferenceType.getInstance(ClassType.OBJECT, ClassType.STRING).arrayTypeAsType();
//		Type type2 = UncertainReferenceType.getInstance(ArrayType.OBJECT_ARRAY, ArrayType.STRING_ARRAY);

		Logger.debug(type1, type2, type1.castToNarrowestNoexcept(type2));
	}
}
